...
sqldf("select * from Posts where  PostTypeId = 1 and FavoriteCount >= 25 and ViewCount >= 10000")
...
pomocnicza <- (sqldf("select * from Posts where  PostTypeId = 1 and FavoriteCount >= 25 and ViewCount >= 10000"))
...
df_sqldf <-function(df1, df2, df3,...){
  ...
  pomocnicza <- (sqldf("select * from Posts where  PostTypeId = 1 and FavoriteCount >= 25 and ViewCount >= 10000"))
  ostateczna2 <- na.omit(sqldf("select Title, Score, ViewCount, FavoriteCount from pomocnicza "))
  ostateczna2
}
df_sqldf(Posts)
